...
The canonical rule catalogue and the executable checker are **bundled in this
skill** — it needs no external checkout to run:
...
Downstream artifacts derive from this catalogue, they are **not** its source:
`typo3-14-gold` is a runnable **reference implementation** that scores 100 %;
`typo3-project-standard` is the human-readable companion narrative. Both are
Netresearch-internal and optional — propose rule changes **here**.
